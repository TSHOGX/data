    ...
    if game_name == "光与夜之恋":
            ...
            if "text" in item:
                ...
                if len(matches) == 1:
                    ...
                    if not "<br />" in card_name:
                        ...
                        video_weibo = (
                            item["video_url"].split(",") if "video_url" in item else []
                        )
        ...
        for item in json_data:
            if "text" in item and "停服更新维护公告" not in item["text"]:
                ...
                if not (
                    len(matches_1) > 0 and len(matches_2) > 0 and len(matches_3) > 0
                ):
                    if len(matches_1) > 0:
...
if GAMENAME == "光与夜之恋":
    with open(
        BILIBILI_JSON_PREFIX + "LightAndNight-01.json", "r", encoding="utf-8"
    ) as file:
        bilibili_data = json.load(file)
    with open(
        BILIBILI_JSON_PREFIX + "LightAndNight-02.json", "r", encoding="utf-8"
    ) as file:
        bilibili_data += json.load(file)
    with open(
        BILIBILI_JSON_PREFIX + "LightAndNight-03.json", "r", encoding="utf-8"
    ) as file:
        bilibili_data += json.load(file)
elif GAMENAME == "恋与深空":
    with open(
        BILIBILI_JSON_PREFIX + "LoveAndDeepspace.json", "r", encoding="utf-8"
    ) as file:
        bilibili_data = json.load(file)
